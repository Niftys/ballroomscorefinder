...
const handleCors = (req: Request, res: Response, handler: () => void) => {
  corsHandler(req, res, () => {
    handler();
  });
};
...
export const fetchCompetitors = functions.https.onRequest((req, res) => {
...
// Fetch competitions
export const fetchCompetitions = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const db = admin.firestore();
  ...
  handleCors(req, res, async () => {
    try {
      ...
      const results = await executeWithDeduplication(cacheKey, async () => {
        ...
        let query: any = db.collection('scores');

      if (competition) {
        // Convert competition names to IDs
        const compNames = (competition as string).split(',');
        const competitionsRef = db.collection('competitions');
      ...
      let competitorSearchTerm: string | null = null;
      let styleSearchTerm: string | null = null;
      
      if (competitor) {
        competitorSearchTerm = (competitor as string).toLowerCase();
        console.log('Main search - Will filter for competitor:', competitorSearchTerm);
      }

      if (style) {
        styleSearchTerm = (style as string).toLowerCase();
      ...
      if (judge) {
        ...
        let judgesSnapshot = await judgesRef
          .where('name', '>=', judge as string)
          .where('name', '<=', (judge as string) + '\uf8ff')
          .limit(50)
          .get();
        
        const searchTerm = (judge as string).toLowerCase();
        ...
        if (matchingJudges.length === 0 && (judge as string).length >= 2) {
          const broaderSnapshot = await judgesRef
            .where('name', '>=', (judge as string).charAt(0))
            .where('name', '<=', (judge as string).charAt(0) + '\uf8ff')
            .limit(100)
            .get();
      ...
      if (score) {
        query = query.where('score', '==', parseInt(score as string));
      }

      if (overall_score) {
        query = query.where('overall_score', '==', parseInt(overall_score as string));
      }
      ...
      if (competitorSearchTerm || styleSearchTerm) {
        ...
        filteredDocs = snapshot.docs.filter((doc: any) => {
      ...
      const fetchInChunks = async (collection: string, ids: string[]) => {
        ...
        try {
          ...
          const results = await Promise.all(
            chunks.map(chunk => 
              db.collection(collection).where(admin.firestore.FieldPath.documentId(), 'in', chunk).get()
            )
          );
...
// Fetch judges
export const fetchJudges = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
...
// Fetch styles
export const fetchStyles = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
...
export const fetchTotalPlacements = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const placement = req.query.placement as string || '1';
...
export const fetchAnalytics = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const competitor = req.query.competitor as string;
      if (!competitor) {
      ...
      const judgesMap = getFromCache('judges_map') || new Map<string, string>();
      const stylesMap = getFromCache('styles_map') || new Map<string, string>();
      const compsMap = getFromCache('comps_map') || new Map<string, string>();
      ...
      const fetchInChunks = async (collection: string, ids: string[], map: Map<string, string>, cacheKey: string) => {
      ...
      if (missingJudgeIds.length > 0) {
        fetchPromises.push(
          fetchInChunks('judges', missingJudgeIds, judgesMap, 'judges_map')
        );
      }
      
      if (missingStyleIds.length > 0) {
        fetchPromises.push(
          fetchInChunks('styles', missingStyleIds, stylesMap, 'styles_map')
        );
      }
      
      if (missingCompIds.length > 0) {
        fetchPromises.push(
          fetchInChunks('competitions', missingCompIds, compsMap, 'comps_map')
        );
      }
      ...
      const judgeAnalytics = new Map<string, {
        judgeName: string,
        totalScore: number,
        scoreCount: number,
        styles: Set<string>
...
export const fetchAveragePlacements = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const competitor = req.query.competitor as string;
      if (!competitor) {
      ...
      const scoresSnapshot = await scoresQuery.get();
      const styleStats: { [key: string]: { total: number, count: number } } = {};
...
export const fetchCompetitionHistory = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const competitor = req.query.competitor as string;
      if (!competitor) {
      ...
      const uniqueEntries = new Map<string, { compName: string, placement: number }>();
      ...
      const competitionGroups = new Map<string, Map<number, number>>();
      
      for (const entry of uniqueEntries.values()) {
        if (!competitionGroups.has(entry.compName)) {
          competitionGroups.set(entry.compName, new Map<number, number>());
        }
...
export const fetchUserCompetitionHistory = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      ...
      if (req.query.aliases) {
        try {
          const aliasesStr = decodeURIComponent(req.query.aliases as string);
      ...
      allPeopleSnapshot.docs.forEach(doc => {
        ...
        for (const searchTerm of searchTermsLower) {
          ...
          if (searchTerm.includes(' & ')) {
            const [termFirst, termLast] = searchTerm.split(' & ').map((s: string) => s.trim());
            
            // Check if personName contains this couple (could be "Sam Moran & Partner" or "Partner & Sam Moran")
            if (personName.includes(' & ')) {
              const [personFirst, personLast] = personName.split(' & ').map((s: string) => s.trim());
      ...
      const allScores: admin.firestore.QueryDocumentSnapshot[] = [];
...
export const fetchUserScores = functions.https.onRequest((req, res) => {
  handleCors(req, res, async () => {
    try {
      const firstName = req.query.firstName as string;
      const lastName = req.query.lastName as string;
      const aliases = req.query.aliases ? JSON.parse(req.query.aliases as string) : [];
